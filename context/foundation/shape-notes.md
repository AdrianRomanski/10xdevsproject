...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "kategoria bólu"
      decision: "brak zdolności/możliwości — nic dziś nie śledzi postępu kulinarnego"
    - topic: "insight"
      decision: "gamifikacja postępu przez różnorodność (kuchnie/składniki), nie liczbę zapisanych przepisów"
    - topic: "zasięg persony"
      decision: "pojedynczy nazwany użytkownik — sam autor projektu"
    - topic: "model dostępu"
      decision: "login (email+hasło/OAuth/passwordless), płaski model kont — jeden typ konta"
    - topic: "pierwszy przepływ MVP"
      decision: "login → dodaj potrawę (nazwa, kuchnia, składniki/przyprawy) → naliczenie XP → aktualizacja gradientu fartucha; mieści się w ≤3 tygodniach"
    - topic: "FR i priorytety"
      decision: "9 FR zatwierdzonych: FR-001..006 must-have (rdzeń, postęp, konto), FR-007..009 nice-to-have (osiągnięcia, specjalizacje, AI-sugestie)"
    - topic: "runda Sokratejska (FR-001..009)"
      decision: "każdy FR przeszedł jeden kontrargument; rozstrzygnięcia: FR-002 (XP tylko za unikalne potrawa+kuchnia), FR-003 (gradient + liczba XP + etykieta), FR-004 (kuratorowana lista kuchni + licznik potraw), FR-005 (autouzupełnianie zamiast wolnego tekstu), FR-007 (progi oparte na liczniku z FR-004), FR-008 (specjalizacja = ręczna odznaka, nie duplikat licznika); FR-001, FR-006, FR-009 — bez zmian"
  frs_drafted: 9
...
> Uwaga na przyszłość: przy znacznie większej skali (wielu użytkowników) naturalnym rozszerzeniem byłyby elementy społecznościowe (rankingi, porównania postępów) — to nowa funkcja poza obecną regułą domenową, niezwiązana z MVP.
...
**Timeline budget:** mvp_weeks: 3 (pierwszy przepływ potwierdzony jako wykonalny w ≤3 tygodnie pracy po godzinach)
...
- FR-009: System sugeruje nowe potrawy do wypróbowania (wsparcie AI). Priority: nice-to-have
  > Socrates: Kontrargument rozważony: "zależność od zewnętrznego serwisu (web search/API) to ryzyko blokujące." Rozstrzygnięcie: FR-009 zostaje jako rozszerzenie po MVP; konkretny mechanizm (ogólny web search vs integracja z API przepisów) pozostaje otwartym pytaniem i nie blokuje rdzenia produktu.
...
- **Bez własnego silnika rekomendacji potraw** — sugestie AI (FR-009) opierają się na gotowym mechanizmie (web search / zewnętrzne API), nie na budowanym od zera algorytmie rekomendacji.
...
- Kandydaci do rozważenia przy wyborze mechanizmu AI-sugestii nowych potraw (FR-009, Open Question #1): zewnętrzne API/bazy przepisów, np. Spoonacular, TheMealDB, albo ogólny web search przez asystenta AI. Do rozstrzygnięcia na etapie doboru stacku technologicznego, nie w PRD.
